...
if ($action === 'list_students') {
    ...
    // Retrieve the student information submitted by the form
    $major_id = filter_input(
        INPUT_POST,
        'major_id',
        FILTER_VALIDATE_INT
    ...
    $firstname = trim(
        (string) filter_input(INPUT_POST, 'first_name')
    );

    $lastname = trim(
        (string) filter_input(INPUT_POST, 'last_name')
    );

    $gender = trim(
        (string) filter_input(INPUT_POST, 'gender')
    );

    // Check that all form fields contain valid information
    if (
        $major_id === null ||
        $major_id === false ||
        $firstname === '' ||
        ...
        include __DIR__ .
            '/../errors/joshua_rogers_error.php';
    } else {
        ...
        // Return to the student list for the selected major
        header(
            'Location: .?action=list_students&major_id=' .
            urlencode((string) $major_id)
        );
